...
for(UBYTE y = 0; y < MAP_TILE_HEIGHT; ++y) {
  for(UBYTE x = 0; x < MAP_TILE_WIDTH; ++x) {
    // i juÅ¼ masz zmienne x,y ktÃ³re siÄ™ ruszajÄ…
    switch (kamyki[x][y])    {
...
s_pBmText = fontCreateTextBitMap(200, s_pFont->uwHeight); // bitmapa robocza dÅ‚uga na 200px, wysoka na jednÄ… liniÄ™ tekstu
